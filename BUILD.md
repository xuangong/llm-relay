...
本文档说明如何构建 LLM Relay（v0.3.2+）。官方 release 发布 GUI：macOS Universal + Windows x64；发布 TUI/agent：Linux x64 + macOS Apple Silicon + macOS Intel。
...
Universal DMG 会落在：
- `src-tauri/target/universal-apple-darwin/release/bundle/dmg/LLM Relay_0.3.2_universal.dmg`
...
├── nsis\
│   └── LLM Relay_0.3.2_x64-setup.exe     # NSIS 安装器（推荐）
└── msi\
    └── LLM Relay_0.3.2_x64_en-US.msi     # MSI 安装器
```
...
├── deb/
│   └── llm-relay_0.3.2_amd64.deb      # Debian/Ubuntu 包
├── rpm/
│   └── llm-relay-0.3.2-1.x86_64.rpm   # Fedora/RHEL 包
└── appimage/
    └── llm-relay_0.3.2_amd64.AppImage # AppImage
```
...
├── deb/llm-relay_0.3.2_amd64.deb
├── rpm/llm-relay-0.3.2-1.x86_64.rpm
└── appimage/llm-relay_0.3.2_amd64.AppImage
```
...
可以**本地构建 + 手动上传到 GitHub Release**。v0.3.2 的发布就是这么做的。
...
gh release upload v0.3.2 release-assets/* --clobber
```
...
- [ ] draft → publish（在 GitHub web UI 或 `gh release edit v0.3.2 --draft=false`）
