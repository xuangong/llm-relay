...
> **v0.3.2 新增**：TUI（终端界面）+ 无头 agent 守护进程，可在服务器上长期驻留；GUI 与 TUI 自动互斥，GUI 检测到守护进程时支持一键接管。
...
- **macOS GUI**: `LLM Relay_0.3.2_universal.dmg`（Universal，支持 Apple Silicon + Intel）
- **Windows GUI**: `LLM Relay_0.3.2_x64-setup.exe`（NSIS 安装器）
