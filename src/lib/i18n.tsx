...
const en: Translations = {
  common: {
    ...
    relayRunning: "Local proxy running",
    relayStopped: "Local proxy stopped",
    relayUnknown: "Checking local proxy",
    relayPort: "Port {port} in use",
    relayStoppedHint: "No proxy port reserved · Select Use on a gateway to enable",
    retryRestore: "Retry config restore",
  ...
  wsl: {
    ...
    syncPendingShort: "WSL2 sync pending",
    clientDisabled: "Relay disabled",
    loginDetected: "Environment disabled after official login",
    manageHost: "Manage Windows Host with Relay",
    environmentHint: "Independent of WSL. Turning off the last environment also stops Relay and releases its ports.",
    environmentDisabled: "This environment is not managed by Relay",
    environmentLogin: "Official {clients} login detected. Relay management is disabled for this environment.",
    loginCleanupPending: "Sync is disabled for this environment. Configuration cleanup will retry automatically.",
    loginReleased: "Official login detected. New credentials preserved and old backups cleared. Enable to use Relay again.",
    syncFailedShort: "WSL2 sync failed · retry pending",
    statusUnavailable: "WSL2 sync status unavailable",
    openSyncDetails: "Open Settings to view distro sync details",
    notProbed: "Not yet probed.",
  },
  disable: {
    title: "Disable LLM Relay",
    intro: "This stops the local proxy, disconnects active requests, releases its ports, and restores your original CLI configuration. The proxy stays stopped even if configuration restoration fails.",
    nothingToRestore: "No Relay origin files are available to restore.",
    confirm: "Disable",
    cancel: "Cancel",
    success: "Local proxy stopped, ports released, and original CLI config restored.",
    successCleared: "Relay disabled. Relay-written settings cleared.",
    failed: "Failed to disable: {error}",
    restoreFailed: "Local proxy stopped and ports released. Config restoration failed: {error}",
    present: "present",
    absent: "absent",
  },
  addDialog: {
...
const zh: Translations = {
  common: {
    ...
    disableRelay: "停用中继",
    relayRunning: "本地代理运行中",
    relayStopped: "本地代理已停止",
    relayUnknown: "正在查询本地代理状态",
    relayPort: "占用端口 {port}",
    relayStoppedHint: "未占用代理端口 · 在网关上点击使用可启用",
    retryRestore: "重试恢复配置",
  ...
  wsl: {
    ...
    syncPendingShort: "WSL2 等待同步完成",
    clientDisabled: "Relay 已停用",
    loginDetected: "官方登录后已停用对应环境",
    manageHost: "使用 Relay 接管 Windows 本机",
    environmentHint: "与各 WSL 环境独立。最后一个环境关闭时，总开关也会关闭并释放端口。",
    environmentDisabled: "此环境已停用 Relay 接管",
    environmentLogin: "检测到 {clients} 官方登录，已停用此环境的 Relay 接管。",
    loginCleanupPending: "此环境已停止同步；正在恢复配置并清理代理设置，失败后会自动重试。",
    loginReleased: "检测到官方登录，已保留新凭证并清理旧备份。重新勾选此环境可再次使用 Relay。",
    syncFailedShort: "WSL2 同步失败 · 等待重试",
    statusUnavailable: "WSL2 同步状态暂不可用",
    openSyncDetails: "打开设置查看发行版同步详情",
    notProbed: "尚未探测。",
  },
  disable: {
    title: "停用 LLM Relay",
    intro: "将关闭本地代理、断开正在转发的请求、释放监听端口，并恢复原始 CLI 配置。即使配置恢复失败，代理也保持停止。",
    nothingToRestore: "没有可用于恢复的 Relay origin 文件。",
    confirm: "停用",
    cancel: "取消",
    success: "本地代理已停止、端口已释放，原始 CLI 配置已恢复。",
    successCleared: "已停用中继，清除中继写入的字段。",
    failed: "停用失败：{error}",
    restoreFailed: "本地代理已停止、端口已释放，但配置恢复失败：{error}",
    present: "存在",
    absent: "不存在",
  },
  addDialog: {
