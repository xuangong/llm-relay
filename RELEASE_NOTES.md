...
- **本地备份允许自由修改**：取消 `.llm-relay.origin` 和 `.llm-relay.bak` 的 SHA-256 记录与校验，旧清单中的哈希字段会被忽略。修改备份内容不会阻止 Use / Save / WSL 同步；恢复时直接使用备份当前的完整内容。停用时先把工作配置保存为 `.bak`，再恢复 `.origin`；下次 Use 先把当时的工作配置保存为 `.origin`，再恢复已有 `.bak`。清单仍区分原文件不存在与空文件；需要恢复的备份缺失或无法读取时仍会报错。
